...
def main():
	...
	Thread(target = transform_to_upc_job).start()
	#while True:
...
def master_connection_open():
	...
	while True:                                                         #Master automatically issue a thread for the incoming worker
		...
		if (pc_name=='PC1'):
			try:
				job_flag = 1
				Thread(target=check_job_to_send, args=(connection, ip, pc_name, job_flag)).start()        #enter to the available worker list, if job exists, worker can accept that job to be processed.Otherwise, keep connected to Master
			except:
...
def check_job_to_send(connection, ip, pc_name, job_flag):                        #each worker pc obtains a channel and enter to the available worker list
...
def aplas_job_zipping():
		...
		for file_name2 in chk_aplas_jobs:
			...
			if file1 == file2:
				if extension1 !=extension2:
					try:
						#rename_itema = extract_aplas_manifest(APLAS_umount, file1)
						#print (rename_item+"mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm")
						#if rename_item==S1:
						#	rename_itema = "BasicAppX\r\n"
						#elif rename_item == S2:
						#	rename_itema = 'S2'
						#else:
						#	rename_itema = 'S3'
	...
	# for aplas_job in os.listdir(all_job_dir):
	# 	aplas_joba = aplas_job.split('_')
	# 	if aplas_joba[0]==S1:
	# 		os.rename(os.path.join(all_job_dir, aplas_job), os.path.join(all_job_dir,"S1"+'_'+aplas_joba[1]))
	# 	elif aplas_joba[0]==S2:
	# 		os.rename(os.path.join(all_job_dir, aplas_job), os.path.join(all_job_dir,"S2"+'_'+aplas_joba[1]))
	# 	else:
	# 		os.rename(os.path.join(all_job_dir, aplas_job), os.path.join(all_job_dir,"S3"+'_'+aplas_joba[1]))
...
def check_types_job_count():  #get number of jobs for each job types
	S1 = "BasixAppX1"         #types of APLAS jobs
  	S2 = "BasixAppX2"
  	S3 = "ColorGameX"
  	S4 = "SoccerMatch"
	...
	print ("BasixAppX1", typea_count)    #check the number of jobs for each type
	...
	print ("MyLibrary", typef_count)
										 #call the middleware program to get connection with Sam scheduling API by passing parameters
	subprocess.call(['python3', '/home/heinhtet/Desktop/UPC_Master/middleware_to_connect_sam_scheduling_api.py', str(typea_count), str(typeb_count), str(typec_count), str(typed_count), str(typee_count), str(typef_count)])



def transform_to_upc_job():                                          #submitted jobs are renamed to be compactible with the system requirements e.g. currentToken_ddmmyyyy_jobName
	global token_counter_today
	global available_list
...
def check_container_require(zip_name_container, job_dir, sys_name):
	...
	if detect_system[2]=='EPLAS':
		...
		S1 = "BasixAppX1"         #types of APLAS jobs
	  	...
	  	S5 = "AnimalTour"
	  	S6 = "MyLibrary"
		job_data = '/home/heinhtet/Desktop/UPC_Master/data.json'

		if value==S1:
			print ("S1")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S1_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S1_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S1_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S1", job_data, current_job)
		elif value==S2:
			print ("S2")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S2_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S2_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S2_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S2", job_data, current_job)
		elif value==S3:
			print ("S3")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S3_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S3_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S3_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S3", job_data, current_job)
		elif value==S4:
			print ("S4")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S4_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S4_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S4_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S4", job_data, current_job)
		elif value==S5:
			print ("S5")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S5_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S5_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S5_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S5", job_data, current_job)
		else:
			print ("S5")
			shutil.make_archive(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S6_'+detect_systemb[2]+'-'+detect_systema[1], 'zip', job_dir)
			subprocess.call(['rm', '-r', job_dir])
			shutil.move(common_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S6_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip', container_queue)                              #All checked jobs are reached to the container queue and then, moved to the correspondance worker queue
			subprocess.call(['chmod', '777', '-R', container_queue])
			current_job = container_queue+detect_systemb[0]+'_'+detect_systemb[1]+'_S6_'+detect_systemb[2]+'-'+detect_systema[1]+'.zip'
			assign_correspondance_job("S5", job_data, current_job)
...
def assign_correspondance_job(job_type, job_data_json, current_job):   # assign the jobs to the correspondance worker PC queue in accordance with the schedule output json file
	pc_dir = '/home/heinhtet/Desktop/UPC_Master/' 
	if job_type=="S1":
		ff = open(job_data_json, 'r')
		data = json.loads(ff.read())
		pc6_S1_jobs_amount = data['data'][0]['result_details'][0]['job_amount']
		pc6_S1_current_no = check_no_jobs_type_PC("S1", "PC66")
		
		pc5_S1_jobs_amount = data['data'][0]['result_details'][1]['job_amount']
		pc5_S1_current_no = check_no_jobs_type_PC("S1", "PC55")
		
		pc4_S1_jobs_amount = data['data'][0]['result_details'][2]['job_amount']
		pc4_S1_current_no = check_no_jobs_type_PC("S1", "PC44")
		
		pc3_S1_jobs_amount = data['data'][0]['result_details'][3]['job_amount']
		pc3_S1_current_no = check_no_jobs_type_PC("S1", "PC33")

		print (pc6_S1_jobs_amount)
		print (pc6_S1_current_no)

		if int(pc6_S1_current_no)<=int(pc6_S1_jobs_amount):
			shutil.move(current_job, pc_dir+"PC66")
		elif int(pc5_S1_current_no)<=int(pc5_S1_jobs_amount):
			shutil.move(current_job, pc_dir+"PC55")
		elif int(pc4_S1_current_no)<=int(pc4_S1_jobs_amount):
			shutil.move(current_job, pc_dir+"PC44")
		else:
			shutil.move(current_job, pc_dir+"PC33")

		#for a in range(4):
		#	print (data['data'][0]['result_details'][a]['job_amount'])
		
	elif job_type=="S2":
		ff = open(job_data_json, 'r')
		data = json.loads(ff.read())
		pc6_S2_jobs_amount = data['data'][1]['result_details'][0]['job_amount']
		pc6_S2_current_no = check_no_jobs_type_PC("S2", "PC66")

		pc5_S2_jobs_amount = data['data'][1]['result_details'][1]['job_amount']
		pc5_S2_current_no = check_no_jobs_type_PC("S2", "PC55")

		pc4_S2_jobs_amount = data['data'][1]['result_details'][2]['job_amount']
		pc4_S2_current_no = check_no_jobs_type_PC("S2", "PC44")

		pc3_S2_jobs_amount = data['data'][1]['result_details'][3]['job_amount']
		pc3_S2_current_no = check_no_jobs_type_PC("S2", "PC33")

		print (pc6_S2_jobs_amount)
		print (pc6_S2_current_no)

		if int(pc6_S2_current_no)<=int(pc6_S2_jobs_amount):
			shutil.move(current_job, pc_dir+"PC66")
		elif int(pc5_S2_current_no)<=int(pc5_S2_jobs_amount):
			shutil.move(current_job, pc_dir+"PC55")
		elif int(pc4_S2_current_no)<=int(pc4_S2_jobs_amount):
			shutil.move(current_job, pc_dir+"PC44")
		else:
			shutil.move(current_job, pc_dir+"PC33")
		#for a in range(4):
		#	print (data['data'][1]['result_details'][a]['job_amount'])
	else:
		ff = open(job_data_json, 'r')
		data = json.loads(ff.read())
		pc6_S3_jobs_amount = data['data'][2]['result_details'][0]['job_amount']
		pc6_S3_current_no = check_no_jobs_type_PC("S3", "PC66")

		pc5_S3_jobs_amount = data['data'][2]['result_details'][1]['job_amount']
		pc5_S3_current_no = check_no_jobs_type_PC("S3", "PC55")

		pc4_S3_jobs_amount = data['data'][2]['result_details'][2]['job_amount']
		pc4_S3_current_no = check_no_jobs_type_PC("S3", "PC44")

		pc3_S3_jobs_amount = data['data'][2]['result_details'][3]['job_amount']
		pc3_S3_current_no = check_no_jobs_type_PC("S3", "PC33")

		print (pc6_S3_jobs_amount)
		print (pc6_S3_current_no)

		if int(pc6_S3_current_no)<=int(pc6_S3_jobs_amount):
			shutil.move(current_job, pc_dir+"PC66")
		elif int(pc5_S3_current_no)<=int(pc5_S3_jobs_amount):
			shutil.move(current_job, pc_dir+"PC55")
		elif int(pc4_S3_current_no)<=int(pc4_S3_jobs_amount):
			shutil.move(current_job, pc_dir+"PC44")
		else:
			shutil.move(current_job, pc_dir+"PC33")

def check_no_jobs_type_PC(job_type, PC):                      #check jobs are assigned to the correspondace worker PC 
	container_queue = '/home/heinhtet/Desktop/UPC_Master/'
	path_name = ""
	count = 0
	print (job_type, PC)
	#try:
